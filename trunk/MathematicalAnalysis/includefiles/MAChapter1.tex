...
\[
  \frac12g(\enspace)^2,
\]
即先对自变量作平方运算，然后再乘常数~$g/2$，函数的定义域为闭区间~$\Brack{0,\smsqrt{\sfrac{2h}g}}$；
...
例如~$y=\sin x$~在~$\FR$~上是有界的，因为取~$M=1$，对任意~$x\in\FR$，都有~$\abs{\sin x}\leq1$。
...
而曲线不全落在两直线之间，意味着总有一点落在两直线之外。%%可用符号表示为，存在~$x'\in X$，使得~$\abs{f(x')}>M$。
...
反函数还有另一种定义，正如实数~$a$~的倒数有两种定义（一种是：$a\neq0$，称~$\sfrac 1a$~为~$a$~的倒数；令一种是：若存在~$b\in\FR$，%
...
反正切函数~$y=\arctan x\Paren[\Big]{x\in\FR,-\dfrac\pi2<y<\dfrac\pi2}$~的图象为\ref{fig:sec1.2-25}；%
