...
\ProvidesPackage{xeCJK}
  [2012/01/31 v2.4.5 package for typesetting CJK scripts with Xe(La)TeX]
...
\def\xeCJK@StrIfBlankTF#1{
  \ifnum\strcmp{\space}{#1}=\z@
    \expandafter\@firstoftwo
  \else\ifnum\strcmp{\empty}{#1}=\z@
...
\define@key{xeCJK/options}{CJKfntef}[true]{\xeCJK@StrIfEqTF{true}{#1}{
  \unless\ifcsname if@ignore\endcsname
...
\def\xeCJK@newfontfamily@aux#1[#2]#3{
  \expandafter\xdef\csname xeCJK@font@#1\endcsname{#3}
  \xeCJK@new@fontfamily\xeCJK@temp@name[#2]{#3}
...
\def\xeCJK@setfont{\setfontfamily{\@nameuse{CJKfamily@\xeCJK@family}}\selectfont}
\endlinechar `\^^M
\endinput
%%
